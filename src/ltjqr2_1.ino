...
float OutputLQR = 2;    //输出量
//////////////////////////////////////////////////////


///////////////////////五连杆机械臂/////////////////////////
float Theta0,Theta1,Theta2,Theta4,Omiga1,Omiga4;
...
int l0=32;
float Leg_L; 
float Leg_x,Leg_y,Target_x,Target_y;
float cosfoai_01,cosfoai_12,cosfoai_34,foai_01,foai_12,foai_34,foai_40,foai_t;
//////////////////////////////////////////////////////
...
float Ts = 0;     //周期时间
float TsOled = 0;     //Oled周期时间
unsigned long Tt; 
unsigned long now_us = 0;
unsigned long velocity_calc_timestamp = 0;
unsigned long velocity_calc_timestamp_oled = 0;

struct{
 
    float Q_angle; //陀螺仪的噪声协方差
    float Q_bias; //陀螺仪的漂移协方差
    float R_measure; //角度测量噪声协方差
    float Angle; 
    float Angle1; 
    float Bias; 
    float Rate; 
...
float A_Angle = 0;
float B_AngleR = 0;//设置机器旋转角度
float A_AngleR = 0;
float R_Gain = 0;//角度旋转增益
float R_Gain2 = 0;//角度旋转增益2
...
void TIME_INTERRUPT_2();    // 定时中断函数2(高速)用于蓝牙接受指令
...
void A_Velocity_filter(float NewVelocity )//类似于卡尔曼滤波
{
...
void YKalmangGetAngle(float newAngle, float newRate, float dt) {
    ...
    KalmanY.p[0][0] += dt * (dt*KalmanY.p[1][1] - KalmanY.p[0][1] - KalmanY.p[1][0] + KalmanY.Q_angle);//Pk
    KalmanY.p[0][1] -= dt * KalmanY.p[1][1];
    KalmanY.p[1][0] -= dt * KalmanY.p[1][1];
    KalmanY.p[1][1] += KalmanY.Q_bias * dt;

    float S = KalmanY.p[0][0] + KalmanY.R_measure; // Estimate error 估计误差 =Pk*H^2+R

    float K[2]; // Kalman gain - This is a 2x1 vector 卡尔曼增益 K = Pk*H/Pk*H^2+R
    K[0] = KalmanY.p[0][0] / S;
    K[1] = KalmanY.p[1][0] / S;

    float y = newAngle - KalmanY.Angle; // Angle difference 测量误差

    KalmanY.Angle += K[0] * y;//当前估计值=上一次估计值+卡尔曼增益（当前测量值-上一次估计值）
    KalmanY.Bias += K[1] * y;
...
void XKalmangGetAngle(float newAngle, float newRate, float dt) {
    ...
    KalmanX.p[1][1] = KalmanX.p[1][0] + KalmanX.Q_bias * dt;
...
{   
    ...
    Avelocity.newerr1 = A_Sliding_weighted_filter(Avelocity.newerr);//滑动加权滤波

    A_Velocity_filter(Avelocity.newerr1);//第二次滤波
...
{    
  ...
  if (SerialBT.available()) 
  {
    sBus.toChannels = 0;
    //// sBus.UpdateServos();
    // sBus.UpdateChannels();//取消使用sbus读取数据
    sBus.toChannels = 0;

    ///////////////蓝牙模拟sbus//////////////////
    sBus.channels[0] = rxpack.shorts[0];
    sBus.channels[1] = rxpack.shorts[1];
    sBus.channels[2] = rxpack.shorts[2];
    sBus.channels[3] = rxpack.shorts[3];
    sBus.channels[4] = rxpack.shorts[4];
    sBus.channels[5] = rxpack.shorts[5];
...
float A_Sliding_weighted_filter(float xdat) //滑动加权滤波算法
{
  ...
  {
      sample_array[i-1] = sample_array[i];//队列全部往前递进一位
      array_sum += sample_array[i] * i;//重新计算队列和，较近的数据权重高
  }  
  sample_array[n_sample-1] = xdat;//最新的数据填入队列最后一位
  array_sum += xdat * n_sample;//乘以权重，填入总和
  float filte_value= (array_sum / (11*n_sample/2.0)) * 9 / 7.0; //根据总和进行滤波
  return filte_value;
}
...
void Set_servo_Angle(float aangle,float bangle,float aangleR,float bangleR)
{

  float a_angle = aangle;//高度角度
  float b_angle = bangle;
  float a_angleR = aangleR;//旋转角度
  float b_angleR = bangleR;
  
  if(a_angle<-200) a_angle = -200;
  if(a_angle>800) a_angle = 800;

  if(b_angle<-200) b_angle = -200;
  if(b_angle>800) b_angle = 800;

  if(a_angleR<-200) a_angleR = -200;
  if(a_angleR>200) a_angleR = 200;

  if(b_angleR<-200) b_angleR = -200;
  if(b_angleR>200) b_angleR = 200;
  ...
  AH = 140+ah1+ah2-53.9;//高度，用于计算PID参数（自适应）
  ...
  */
  ...
  if(car_ch9==2)
  {
    ledcWrite(channel0,map(b_angle+b_angleR*BH*R_Gain2*0.001, 0, 900, B1angle_mini, B1angle_max));  //B1
    ledcWrite(channel1,map(b_angle-b_angleR*BH*R_Gain2*0.001, 0, 900, B2angle_mini, B2angle_max));  //B2 
    ledcWrite(channel2,map(a_angle-a_angleR*AH*R_Gain2*0.001, 0, 900, A2angle_mini, A2angle_max));  //A2
    ledcWrite(channel3,map(a_angle+a_angleR*AH*R_Gain2*0.001, 0, 900, A1angle_mini, A1angle_max));  //A1
  }
  else if(car_ch9==0)
  {
...
void LegControl(int x,int y)
{
  ...
  cosfoai_34 = ((Leg_x-l0)*(Leg_x-l0)+Leg_y*Leg_y-l3*l3-l4*l4)/(2*l3*l4);
  foai_34 = 2*pi - acos(cosfoai_34);
  Leg_A = l0 - Leg_x;
  Leg_B = l3 * sin(foai_34);
  Leg_C = l4 + l3*cos(foai_34);
  foai_t = acos(Leg_B/sqrt(Leg_B*Leg_B+Leg_C*Leg_C));
  foai_40 = foai_t - asin(Leg_A/sqrt(Leg_B*Leg_B+Leg_C*Leg_C));
  // foai_40 = foai_t + abs(asin(Leg_A/sqrt(Leg_B*Leg_B+Leg_C*Leg_C)));
  Theta1 = 180-foai_01/pi*180;
  Theta2 = 180-foai_40/pi*180;
  ledcWrite(channel0,map(Theta1, 0, 80, B1angle_mini, B1angle_max));  //B1
  ledcWrite(channel1,map(Theta2, 0, 80, B2angle_mini, B2angle_max));  //B2 
}
...
void Servo_Initialization() // ledc初始化程序
{
  ...
  Set_servo_Angle(A_Angle,B_Angle,A_AngleR,B_AngleR);
  delay(1111);
}
...
void OledInt()
{
  ...
  do
  {
    ...
    u8g2.print("轮腿机器人");
    u8g2.setCursor(0, 25);
    u8g2.print("Wheel-Legged Robot");        // Chinese "Hello World"
  }while ( u8g2.nextPage() );
}

void OledInformation1(String a)
{
  u8g2.begin();    //初始化函数
  u8g2.enableUTF8Print();        // 为Arduino print（）函数启用UTF8支持 
  u8g2.setFont(u8g2_font_wqy12_t_gb2312);  //对“你好世界”的所有字形使用chinese2
...
void OledInformation2(String a,String b)
{
  u8g2.begin();    //初始化函数
  u8g2.enableUTF8Print();        // 为Arduino print（）函数启用UTF8支持 
  u8g2.setFont(u8g2_font_wqy12_t_gb2312);  //对“你好世界”的所有字形使用chinese2
...
int Mpu6050Int()
{
  ...
  // verify connection
  a = "Testing connections...";
  b = "//////";
  OledInformation2(a,b);
  a = mpu.testConnection() ? F("MPU6050 connected") : F("MPU6050 disconnected");
  b = mpu.testConnection() ? F("////////") : F("//////");
  OledInformation2(a,b);
  Serial.println(F("Testing device connections..."));
  Serial.println(mpu.testConnection() ? F("MPU6050 connected") : F("MPU6050 disconnected"));
...
////////////////////////蓝牙模拟sbus///////////////////////////////////
  SerialBT.begin("Wheel_Legged_Robot"); //Bluetooth device name
...
void loop() 
{
  ...
  {      
      ...
      if(start_f==1)
      { 
        int Tpwm = (int)Turn_PID (car_ch1,KalmanZ.Rate);//-gy.z/16.4     yaw轴角度控制输出量
      ...
      else 
      {
        ...
        int Apwm =(int)(Abalance_pin(Avelocity_pin(0,Amotor_speed,Bmotor_speed),(KalmanX.Angle-AngleX_bias))*100);
        int Bpwm =(int)(Bbalance_pin(Bvelocity_pin(0,Amotor_speed,Bmotor_speed),(KalmanX.Angle-AngleX_bias))*100);
      }       
      //mpuInterrupt = false;   
...
//    OledShow();
    ...
    {
      float Hpwm = Height_pid(ch4,-(KalmanY.Angle-AngleY_bias));
      A_Angle = c3+Hpwm;
      B_Angle = c3-Hpwm; 
    }
    ...
    if(car_ch5!=1)
      Set_servo_Angle(A_Angle,B_Angle,A_AngleR,B_AngleR);
    if(abs(KalmanX.Angle-AngleX_bias)>30)
    {
      jdbz_press_time1++; 
    ...
    if(abs(KalmanX.Angle-AngleX_bias)<1)
    {
       jdbz_press_time++;  
